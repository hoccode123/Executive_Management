...
class Category_Controller extends Admin{
    async getList(req, res){
        ...
        //     email
        // } = req.query
        const data = await Category_Model.getList()
        //console.log(data);
    ...
    async create(req, res){
        ...
        // const checkRole = await this.checkModelField(userLogin.data.role, this.roleList());
        // if(!checkRole) return this.response(res, 610,'', '');
        // for (let index = 0; index < userLogin.data.role.length; index++) {
        //     const element = userLogin.data.role[index];
        //     // check co quyen hay khong
        //     if(this.roleList.includes(element)){

        //     }
        // }

        // if(userLogin.data.role.includes('ADMIN')){

        // }

        //return
        //console.log(userLogin.data.role);
        const {
            title,
            slug,
            products,
        ...
        const check_title_exist = await Category_Model.getField({title})
        if(check_title_exist.length != 0) return this.response(res, 605, '', 'title')
        const check_slug_exist = await Category_Model.getField({slug})
    ...
    async update(req,res){
       ...
       const oldData = await Category_Model.getField({_id: new mongoose.Types.ObjectId(req.params.id)})
       //const avatar = oldData[0]['avatar']
        await Category_Model.update({_id: new mongoose.Types.ObjectId(req.params.id)}, req.body)
       
        const data = await Category_Model.getField({_id: new mongoose.Types.ObjectId(req.params.id)})
        return this.response(res, 200, data);
    ...
    async getID(req, res){
        const id = req.query
        const array = await Category_Model.getField(id, 'title slug products type desription content status metaTitle metaDescription schemaStart createDate updateDate createByID updateByID')
        const data=[]
    ...
    async active(req, res){
        ...
        // 4. check db
        const check_id_exist = await Category_Model.getField({_id: {$ne: new mongoose.Types.ObjectId(id)}})
        ...
        await Category_Model.update({_id: new mongoose.Types.ObjectId(id)}, {active: status.toLowerCase()=='on'?true:false}) 
        const data = await Category_Model.getField({_id: new mongoose.Types.ObjectId(id)}, '_id email active createDate createByID')
        return this.response(res,200, data)
    }
    ...
    async delete(req, res){
       ...
       //console.log(_id)
       //; if(Category_Model.getID(_id)=='') return this.response(res, 404)
       const checkRole = await this.checkModelFieldArray(userLogin.data.role, this.roleList());
        if(!checkRole) return this.response(res, 610,'', '');
       
        const data = await Category_Model.delete(req.params.id)
